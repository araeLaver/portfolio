...
public class DataInitializer implements CommandLineRunner {
    ...
    @Value("${app.project.beam-live-url:}")
    private String beamLiveUrl;

    @Value("${app.project.travelmate-live-url:}")
    private String travelmateLiveUrl;

    @Value("${app.project.idea-manager-live-url:}")
    private String ideaManagerLiveUrl;
    ...
    public void run(String... args) throws Exception {
        ...
        beam.setLiveUrl(resolveLiveUrl(beamLiveUrl));
        beam.setStack(Arrays.asList("Java 17", "Spring Boot 3.2", "WebSocket (STOMP)", "Spring Security", "PostgreSQL", "Docker", "Cloud VPS"));
        ...
        travelMate.setLiveUrl(resolveLiveUrl(travelmateLiveUrl));
        ...
        ideaManager.setLiveUrl(resolveLiveUrl(ideaManagerLiveUrl));
    ...
    private void updateProjectUrls() {
        projectRepository.findById("beam-messenger").ifPresent(beam -> {
            String newBeamUrl = resolveLiveUrl(beamLiveUrl);
            if ((newBeamUrl == null && beam.getLiveUrl() != null) || (newBeamUrl != null && !newBeamUrl.equals(beam.getLiveUrl()))) {
                beam.setLiveUrl(newBeamUrl);
                projectRepository.save(beam);
                logger.info("Updated BEAM project liveUrl to: {}", newBeamUrl);
            }
        });

        projectRepository.findById("travelmate").ifPresent(travelMate -> {
            String newTravelmateUrl = resolveLiveUrl(travelmateLiveUrl);
            if ((newTravelmateUrl == null && travelMate.getLiveUrl() != null) || (newTravelmateUrl != null && !newTravelmateUrl.equals(travelMate.getLiveUrl()))) {
                travelMate.setLiveUrl(newTravelmateUrl);
                projectRepository.save(travelMate);
                logger.info("Updated TravelMate project liveUrl to: {}", newTravelmateUrl);
            }
        });

        projectRepository.findById("idea-manager").ifPresent(ideaManager -> {
            String newIdeaManagerUrl = resolveLiveUrl(ideaManagerLiveUrl);
            if ((newIdeaManagerUrl == null && ideaManager.getLiveUrl() != null) || (newIdeaManagerUrl != null && !newIdeaManagerUrl.equals(ideaManager.getLiveUrl()))) {
                ideaManager.setLiveUrl(newIdeaManagerUrl);
                projectRepository.save(ideaManager);
                logger.info("Updated Idea Manager project liveUrl to: {}", newIdeaManagerUrl);
            }
        });
    }
